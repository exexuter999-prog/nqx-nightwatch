...
* 費用込み(手数料 $0.50/枚/片道)で滑り 0/1/2 tick のどれでも改善(**+2.13 / +2.06 /
  +2.00R**)、最大 DD は不変。**セットアップ単位の優位は未確定**(P(improve)=0.73)。
...
* **`selection` = LIVE**(2026-09-20 採用。詳細は `docs/R122_SELECTION_STAGE.md`):
  ...
  市場全体・口座全体の停止は primary を選んだ後に当たるので迂回できない。費用込み
  (手数料 $0.50/枚/片道)で滑り 0/1/2 tick のどれでも改善(+2.13 / +2.06 / +2.00R)、
  最大 DD は不変。**セットアップ単位の優位は未確定**(+0.038R/setup、区間 [−0.05,+0.15]、
  P(improve)=0.73)なので「勝率が上がる」とは言わない。**13/15 で建てる方向が反転する**
  のが最大の変化。戻すのは `mode` を `OFF` にする 1 語。

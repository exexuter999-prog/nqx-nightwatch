...
| `selection` | **OFF** | 高得点 WATCH より ARMED を優先する選択層の変更。実測済み・人の判断待ち |
...
| **監査バンドル自身の確定 3 分足から再構成** | 3,980 本 → **15 分足 779 本** | **使う**(下の検算に合格) |
| Databento 1m / 3m コーパス | 15 分足 2,232 本を作れるが、直接取得 15 分足との**重なりが 0 本** | 正解と突き合わせられない → **使わない** |

**再構成の検算**: 5 本の確定 3 分足がそろった窓だけを 15 分足にし、直接取得した 15 分足と
重なる **50 本すべてで OHLC の差が 0.00pt**(不一致 0)。これは「上位足を 3 分足で代用する」
ことではなく、同じ市場の同じ確定足を足し合わせた**再構成**で、正しさを正解データで確かめてから
使っている。1 本でも欠けた窓は作らない。合わなければ使わない(ズラして合わせることはしない ——
R102 のロール事故と同じ形を作らないため)。
...
すると「並べ替えの後に武装できる候補を優先する」1 行が入る。既定 OFF。
...
* **`shallowCandidate` = SHADOW**:
  ...
  同じ再生でそのまま比較できる。
* **`selection` = OFF**:
  逐次は A 比 **+1.20R**(+7.71 → +8.91)だが、セットアップ単位は −0.009R/setup で区間が
  0 を跨ぎ(P(improve)=0.42)、利得は「取る本数が 37 → 48 に増えたこと」から来ている。
  さらに**同じ周期で建てる方向が反転しうる**(高得点 WATCH の BREAKER BUY を捨てて
  ARMED の TURTLE SELL を採る等)。現行の選択契約を変える判断なので、実測を出したうえで
  人の判断を待つ。
...
| U5 | 選択層の変更は判断待ち | 方向が反転しうる変更で、セットアップ単位の優位は出ていない | 該当 28 周期の内訳(どの候補をどの候補に替えたか)を見て、人が可否を決める |
...
python replay_structure_context.py --split --source recon

...
### P — forecast cone (display-only market-dynamics distribution)

```text
P|h=<1..24 bars>|u=<volatility unit>|q50=~low..~high|q80=~low..~high|q95=~low..~high|med=~price|b=<generation basis>|un=<limitations>
```

The `P` record carries a **distribution**, not a forecast of price. It states the
band the path would occupy given the volatility the market has already printed,
together with the declared levels that act as friction. It is generated from
`O.c` and `Z` alone — it never reads a scenario, trigger, or direction — which is
why it remains valid at `G.n=0`, the state where a scenario-derived `S.pc` tape
cannot exist at all.

- `h` is the horizon in bars, `1..24`. Longer horizons are not supported.
- `u` is the True Range unit the cone was generated from, written as a bare
  decimal — it is a measured quantity, not an estimate. When the packet carries
  at least four `O.c` bars, `u` must agree with that tape's own trailing True
  Range mean within 2% (or 0.25pt, whichever is larger). A cone scaled to a
  different tape than the packet's own is not auditable.
- `q50`, `q80`, `q95` are the confidence bands and `med` the distribution
  median. **Every bound carries `~`**, exactly as `S.pc` prices do: these are
  simulated values and must never read as observed or executable prices.
- Bands must nest: `q95lo <= q80lo <= q50lo <= med <= q50hi <= q80hi <= q95hi`.
- `med` must sit within 25% of the 95% band width from the latest observed
  close. A median far off the last close is a directional call wearing a
  distribution's clothes — the step sample must be centred before simulating.
- `b` states the generation basis; declaring a drift term is an error. The cone
  expresses spread only. Extrapolating recent direction is precisely what turns
  a distribution into a "price will rise" claim.
- `un` states the limitations, explicitly including that the cone is
  display-only and is not a prediction of price.

The cone never feeds current price, level discovery, trigger or confirmation
state, confidence, grade, probability, R:R, stop placement, candidate detection,
event logic, or any validator decision about a scenario. A receiver that does
not know `P` retains it as an unknown record rather than dropping it.
...
2c. When `G.rn` declares `POLICY=WG-H1`, the legacy two-path fallback is replaced
    ...
    for `M.rg=TR|EX` with `S.su=BC|PC`. It also requires frozen `M.rm`
    regime evidence under `policy=RG-H1`. `mode=METRIC` requires `cmp>=20`,
    every `rvr/rar/gar/rvp/rap/dep/ovp` value, matched
    `session/clock/prior`, `bias`, `structure`, and `stable>=2`; the validator
    then applies the versioned TR/EX thresholds. `mode=SCREENSHOT` is TR-only,
    requires 3M/15M/45M source frames plus aligned/accepted/stable structure,
    and requires every emitted scenario to declare `S.ha=A`. It requires 15 exact accepted `O.c`
...
2e. The receiver's operator-only `WG SESSION RISK CAP` is not encoded as a
    ...
    blocks local `EXECUTE` when absent or when one MNQ exceeds the cap. The cap
    is locked once to the packet's symbol/date/session. `EXECUTE` accepts only
    `S.st=C`, re-encodes and revalidates the whole NQX packet, requires a live
    receiver snapshot, then freezes entry/SL/TP/invalidation/side, MNQ quantity,
    stress risk, and packet fingerprint. A frozen operation cannot widen its
    stop or alter geometry through the UI.
2f. `G.ip` is executable posture, not commentary: it must include current numeric
    location, an action now, the next-30-minute observable, and a reason. When a
    scenario remains valid beyond an actual `E.rt`, `S.eh` must include a timed
    `FLAT/CLOSE/REDUCE` deadline no later than the release.
...
13. Every `P` band bound and `P.med` begins with `~`; the cone is a display-only
    distribution and cannot alter a gate, audit, decision, or outcome. Its bands
    must nest, `P.u` must match the packet's own `O.c` trailing True Range mean,
    and `P.med` must stay within 25% of the 95% band width from the latest
    observed close — an off-centre median is a direction forecast, not a spread.
    `~` on `P` means the same thing it means on `S.pc` (simulated, never
    observed), which is a different meaning from `~` on `O.c` (an observed bar
    read against a grid line).

## Minimal valid packet (NO TRADE)

```text
!NQX/1
M|sy=MNQ1!|tf=15M|at=2026-07-14 17:10 JST|ss=CME|sf=3M,15M,45M|dq=M|px=29574.75|qm=H|na=rm,sn,pe,br,pp,ui,bp,ge,cm,ne,ce,sz
